<script lang="ts">
    ...
    const _paperclipSvg = `<?xml version="1.0" encoding="utf-8"?><!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
  <svg fill="#ffffff" viewBox="0 0 36 36" version="1.1"  preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    const paperclipUrl = `data:image/svg+xml;utf8,${encodeURIComponent(_paperclipSvg)}`;
    ...
    const styles = ['photorealistic', 'cinematic', 'digital-art', 'watercolor', 'anime', 'comic', 'cartoon', 'pixelated', 'sculpture'];
    ...
    async function generate() {
      error = null;
      if (!prompt.trim()) { error = 'Please enter a prompt.'; return; }
        loading = true;
        ...
        try {
            ...
            history = generatedImage ? [generatedImage, ...history].slice(0, 8) : history;
        } catch (err) {
            console.error(err);
  ...
  <main class="grid">
    <section class="controls">
      <label class="field">
        ...
        <div class="examples">
          {#each examples as e}
            <button class="chip" type="button" on:click={() => pickExample(e)}>{e}</button>
          {/each}
        </div>
      </label>
      ...
      <div class="row file-row">
        <label class="filelabel" title="Attach reference image">
          <input type="file" accept="image/*" on:change={handleFileChange} />
          <img src={paperclipUrl} alt="paperclip" class="paperclip" />
          <span>{image ? 'Change reference' : 'Attach reference (optional)'}</span>
        </label>
        ...
        <div class="controls-right">
          ...
          {#if generatedImage}
            <button class="btn" on:click={downloadImage} title="Download generated image">Download</button>
          {/if}
        </div>
      </div>
...
<style>
  ...
  .filelabel img.paperclip{ width:14px; height:14px; display:inline-block; object-fit:contain; vertical-align:middle; margin-right:4px; }
  ...
  .error{ color:#ffb4b4; margin:6px 0 0; font-size:.92rem; }
